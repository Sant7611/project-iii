...
from blog.serializers.post_serializer import (
    PostSerializer,
    PostCreateUpdateSerializer,
    PostModerationSerializer,
    OwnerPostListSerializer,
    OwnerPostSerializer
)
...
class PostView(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        NotificationService.send_to_reviewers(
            {
                "title": "Post awaiting review",
                "body": f'"{post.title}" was submitted for moderation.',
                "notification_type": Notification.NotificationChoices.POST_PENDING,
            }
        )
    ...
    def get_queryset(self):
        ...
        if user.is_authenticated and user.role in ["moderator", "super_admin"]:
            return queryset
        ...
        return queryset.filter(approval_status=Post.PostStatus.APPROVED)
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        if request.user.is_authenticated and request.user.role in [
            ...
            serializer = OwnerPostSerializer(
    ...
    def accept(self, request, pk=None):
        ...
        return success_response(
            data={"message": "post approved", "post": serializer.data}, status=200
        )
    ...
    def reject(self, request, pk=None):
        ...
        serializer = self.get_serializer(post)
        return success_response(
            data={"message": "post rejected ", "post": serializer.data}, status=200
        )
