...
class Comment_View(viewsets.ModelViewSet):
    queryset = Comment.objects.all().select_related('post', 'author', 'author__profile', 'parent').prefetch_related('replies__author__profile').filter(post__approval_status='approved')
